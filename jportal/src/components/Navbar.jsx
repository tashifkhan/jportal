...
function Navbar() {
  ...
  return (
    <div className='flex items-center justify-between sm:gap-0 gap-2 w-screen fixed bottom-0 left-0 bg-secondary py-4 px-4'>
      {navItems.map((item) => (
        <NavLink
          key={item.name}
          to={item.path}
          className={({ isActive }) => `
            flex-1 text-md text-foreground text-clip overflow-hidden whitespace-nowrap
            ${isActive ? 'opacity-100' : 'opacity-70'}
          `}
        >
          <div className='flex flex-col items-center'>
            <div className='hover:bg-accent2 rounded-xl w-full p-1 flex items-center justify-center'>
