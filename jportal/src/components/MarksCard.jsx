...
export default function MarksCard({
  ...
  return (
    <div
      className={`${
        useCardBackgrounds ? "bg-background rounded-2xl shadow-sm" : ""
      } px-6 py-5 flex flex-col gap-2 mb-2 border border-border`}
    >
      <div className="flex items-center justify-between gap-4 mb-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-sm font-light truncate text-foreground font-sans">
              {course.name}
            </h3>
            {isAudit && (
          ...
          <div className="text-xs font-normal text-muted-foreground flex items-center gap-3">
            <span>{course.code}</span>
            {credits !== "-" && (
              <span className="text-xs bg-primary/10 text-foreground font-medium rounded px-2 py-0.5">
                {credits} Credits
              </span>
            )}
            {grade !== "-" && (
              <span className="text-xs bg-accent/20 text-foreground font-medium rounded px-2 py-0.5">
                Grade: {grade}
              </span>
            )}
          </div>
